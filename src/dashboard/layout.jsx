...
import { FiHome, FiPlusSquare, FiList, FiUser, FiLogOut, FiMenu, FiX } from 'react-icons/fi';
...
const Layout = ({ children }) => {
  ...
  return (
    <div className={`min-h-screen w-full fixed inset-0 overflow-y-auto ${darkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-blue-50 to-indigo-100'}`}>
      {/* Sidebar */}
      <div
      ...
      >
        <div className={`p-4 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'} flex items-center justify-center`}>
          {sidebarCollapsed ? (
        ...
        <div className="p-4">
          <nav className="space-y-2">
            ...
            <motion.button
              ...
              className={`w-full flex items-center ${sidebarCollapsed ? 'justify-center px-0 py-4' : 'px-4 py-3'} rounded-lg transition-all relative overflow-hidden group ${
            ...
            >
              <RiDashboardFill className={`${sidebarCollapsed ? 'text-xl' : 'mr-3'}`} />
              {!sidebarCollapsed && (
                <motion.span 
                  initial={{ opacity: 1 }}
                  animate={{ opacity: 1 }}
              ...
              <span className={`absolute left-0 bottom-0 h-1 bg-gradient-to-r from-purple-400 to-indigo-500 transition-all duration-300 ${
                location.pathname.includes('dashboard') ? 'w-full' : 'w-0 group-hover:w-full'
              }`}></span>
            </motion.button>
            ...
            >
              <FiPlusSquare className={`${sidebarCollapsed ? 'text-xl' : 'mr-3'}`} />
              {!sidebarCollapsed && (
                <motion.span 
                  initial={{ opacity: 1 }}
                  animate={{ opacity: 1 }}
              ...
              <span className={`absolute left-0 bottom-0 h-1 bg-gradient-to-r from-purple-400 to-indigo-500 transition-all duration-300 ${
                location.pathname.includes('addq') ? 'w-full' : 'w-0 group-hover:w-full'
              }`}></span>
            </motion.button>
            ...
            >
              ...
              <span className={`absolute left-0 bottom-0 h-1 bg-gradient-to-r from-purple-400 to-indigo-500 transition-all duration-300 ${
                location.pathname.includes('allq') ? 'w-full' : 'w-0 group-hover:w-full'
              }`}></span>
            </motion.button>
          </nav>
        </div>
        ...
        <div className={`absolute bottom-0 left-0 right-0 p-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          <button
            onClick={() => navigate('/')}
            className={`w-full flex items-center ${sidebarCollapsed ? 'justify-center px-0 py-4' : 'px-4 py-3'} rounded-lg transition-all ${
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        className={`fixed z-50 ${sidebarCollapsed ? 'left-20' : 'left-64'} top-6 p-2 rounded-full ${
